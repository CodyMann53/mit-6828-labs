...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 0)
	{
		panic("Page fault from within the kernel. VA: %08x", fault_va);
	}
	...
	{
		cprintf("[%08x] user fault va %08x ip %08x\n. Overflow of user exception stack.\n",
			curenv->env_id, fault_va, tf->tf_eip);
		print_trapframe(tf);
		env_destroy(curenv);
	}
	...
	// If already on the user exception stack
	if (tf->tf_esp > UXSTACKTOP-PGSIZE && 
		tf->tf_esp <= UXSTACKTOP-1)
	{
		tf->tf_esp-= 4;
		utf.utf_esp = tf->tf_esp;
		*(struct UTrapframe *)(tf->tf_esp - sizeof(struct UTrapframe)) = utf; 
		tf->tf_esp-= sizeof(struct UTrapframe);
	}
	else
	{
		*(struct UTrapframe *)(UXSTACKTOP - sizeof(struct UTrapframe)) = utf; 
