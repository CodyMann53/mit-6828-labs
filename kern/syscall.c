...
sys_exofork(void)
{
	...
	int retVal = env_alloc(&newenv_store, curenv->env_id);
	if (retVal != 0)
	{
		return retVal;
	}
...
sys_env_set_status(envid_t envid, int status)
{
	...
	int retVal = envid2env(envid, &env_store, 1);
	if (retVal != 0)
	{
		return retVal;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	retVal = page_insert(env_store->env_pgdir, newPage, va, perm);
	if (retVal != 0)
	{
		page_free(newPage);
		return retVal;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int retVal = envid2env(srcenvid, &srcenv_store, 1);
	if (retVal != 0)
	{
		return retVal;
	}
	...
	retVal = envid2env(dstenvid, &dstenv_store, 1);
	if (retVal != 0)
	{
		return retVal;
	}
	...
	struct PageInfo * page = page_lookup(srcenv_store->env_pgdir, srcva, &pte_store);
	if (page == NULL)
	{
		return -E_INVAL;
	}
	...
	retVal = page_insert(dstenv_store->env_pgdir, page, dstva, perm);
	if (retVal != 0)
	{
		return retVal;
	}
